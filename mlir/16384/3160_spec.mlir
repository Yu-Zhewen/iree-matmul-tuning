module attributes {iree_codegen.tuning_spec_with_default_entrypoint, transform.with_named_sequence} {
  ...
  transform.named_sequence @match_mm_16384$async_dispatch_0_matmul_16384x16384x16384_bf16xbf16xf32(%arg0: !transform.any_op {transform.readonly}) -> (!transform.any_op, !transform.any_param) {
    ...
    transform.iree.match.dims_equal %m_dims, [16384] : !transform.param<i64>
    transform.iree.match.dims_equal %n_dims, [16384] : !transform.param<i64>
    transform.iree.match.dims_equal %k_dims, [16384] : !transform.param<i64>
    %0 = transform.param.constant #iree_codegen.compilation_info<lowering_config = #iree_gpu.lowering_config<{mma_kind = #iree_gpu.mma_layout<MFMA_F32_16x16x32_BF16>, promote_operands = [0, 1], promotion_types = [#iree_gpu.use_global_load_dma, #iree_gpu.use_global_load_dma], reduction = [0, 0, 1], subgroup = [4, 8, 0], workgroup = [256, 256, 0]}>, translation_info = <pipeline = LLVMGPUTileAndFuse workgroup_size = [512, 1, 1] subgroup_size = 64, {gpu_pipeline_options = #iree_gpu.pipeline_options<prefetch_num_stages = 2, no_reduce_shared_memory_bank_conflicts = true>, llvm_func_attrs = {"amdgpu-waves-per-eu" = "2"}}>> -> !transform.any_param
  ...
  transform.named_sequence @__kernel_config(%arg0: !transform.any_op {transform.consumed}) -> !transform.any_op attributes {iree_codegen.tuning_spec_entrypoint} {
    %updated_root = transform.foreach_match in %arg0 
        @match_mm_16384$async_dispatch_0_matmul_16384x16384x16384_bf16xbf16xf32 -> @apply_op_config : (!transform.any_op) -> !transform.any_op
