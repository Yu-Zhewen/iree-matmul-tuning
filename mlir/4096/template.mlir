...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  hal.executable private @mm_4096$async_dispatch_0 {
    hal.executable.variant public @rocm_hsaco_fb target(#executable_target_rocm_hsaco_fb) {
      hal.executable.export public @mm_4096$async_dispatch_0_matmul_4096x4096x4096_bf16xbf16xf32 ordinal(0) layout(#pipeline_layout) count(%arg0: !hal.device) -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        hal.return %x, %y, %z : index, index, index
      }
      builtin.module {
        func.func @mm_4096$async_dispatch_0_matmul_4096x4096x4096_bf16xbf16xf32() {
          ...
          %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c0) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<4096x4096xbf16>>
          %2 = hal.interface.binding.subspan layout(#pipeline_layout) binding(2) alignment(64) offset(%c0) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<4096x4096xbf16>>
          %3 = iree_tensor_ext.dispatch.tensor.load %0, offsets = [0, 0], sizes = [4096, 4096], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<4096x4096xbf16>> -> tensor<4096x4096xbf16>
          ...
          %5 = tensor.empty() : tensor<4096x4096xbf16>
          %6 = tensor.empty() : tensor<4096x4096xf32>
          %7 = linalg.fill ins(%cst : f32) outs(%6 : tensor<4096x4096xf32>) -> tensor<4096x4096xf32>
          %8 = linalg.matmul indexing_maps = [#map, #map1, #map2] ins(%3, %4 : tensor<4096x4096xbf16>, tensor<4096x4096xbf16>) outs(%7 : tensor<4096x4096xf32>) -> tensor<4096x4096xf32>
          %9 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel"]} ins(%8 : tensor<4096x4096xf32>) outs(%5 : tensor<4096x4096xbf16>) {
          ^bb0(%in: f32, %out: bf16):
            %10 = arith.truncf %in : f32 to bf16
            linalg.yield %10 : bf16
          } -> tensor<4096x4096xbf16>
          iree_tensor_ext.dispatch.tensor.store %9, %2, offsets = [0, 0], sizes = [4096, 4096], strides = [1, 1] : tensor<4096x4096xbf16> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<4096x4096xbf16>>
          return
        }
      }
    }
  }
  util.func public @mm_4096$async(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view, %arg2: !hal.fence, %arg3: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    %c0 = arith.constant 0 : index
    %c33554432 = arith.constant 33554432 : index
    %c4096 = arith.constant 4096 : index
    %element_type_bf16 = hal.element_type<bf16> : i32
    %dense_row_major = hal.encoding_type<dense_row_major> : i32
    hal.buffer_view.assert<%arg0 : !hal.buffer_view> message("tensor") shape([%c4096, %c4096]) type(%element_type_bf16) encoding(%dense_row_major)
    %0 = stream.tensor.import on(#hal.device.affinity<@__device_0>) %arg0 : !hal.buffer_view -> tensor<4096x4096xbf16> in !stream.resource<external>{%c33554432}
    %1 = stream.timepoint.import on(#hal.device.affinity<@__device_0>) %arg2 : (!hal.fence) => !stream.timepoint
    hal.buffer_view.assert<%arg1 : !hal.buffer_view> message("tensor") shape([%c4096, %c4096]) type(%element_type_bf16) encoding(%dense_row_major)
    %2 = stream.tensor.import on(#hal.device.affinity<@__device_0>) %arg1 : !hal.buffer_view -> tensor<4096x4096xbf16> in !stream.resource<external>{%c33554432}
    %result, %result_timepoint = stream.resource.alloca uninitialized on(#hal.device.affinity<@__device_0>) await(%1) => !stream.resource<external>{%c33554432} => !stream.timepoint
    %3 = stream.cmd.execute on(#hal.device.affinity<@__device_0>) await(%result_timepoint) => with(%0 as %arg4: !stream.resource<external>{%c33554432}, %2 as %arg5: !stream.resource<external>{%c33554432}, %result as %arg6: !stream.resource<external>{%c33554432}) {
      stream.cmd.dispatch @mm_4096$async_dispatch_0::@rocm_hsaco_fb::@mm_4096$async_dispatch_0_matmul_4096x4096x4096_bf16xbf16xf32 {
        ro %arg4[%c0 for %c33554432] : !stream.resource<external>{%c33554432},
        ro %arg5[%c0 for %c33554432] : !stream.resource<external>{%c33554432},
    ...
    %4 = stream.tensor.export on(#hal.device.affinity<@__device_0>) %result : tensor<4096x4096xbf16> in !stream.resource<external>{%c33554432} -> !hal.buffer_view
    util.return %4 : !hal.buffer_view
  }
  util.func public @mm_4096(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub} {
    %0 = util.null : !hal.fence
    %c-1_i32 = arith.constant -1 : i32
    %c0 = arith.constant 0 : index
    %device_0 = hal.devices.get %c0 : !hal.device
    %fence = hal.fence.create device(%device_0 : !hal.device) flags("None") : !hal.fence
    %1 = util.call @mm_4096$async(%arg0, %arg1, %0, %fence) : (!hal.buffer_view, !hal.buffer_view, !hal.fence, !hal.fence) -> !hal.buffer_view
