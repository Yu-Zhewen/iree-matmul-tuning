    ...
    hal.executable.variant public @rocm_hsaco_fb target(#executable_target_rocm_hsaco_fb) {
      ...
      builtin.module {
        func.func @mm_512$async_dispatch_0_matmul_512x512x512_bf16xbf16xf32() {
          ...
          %0 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c0) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<512x512xbf16>>
          %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c0) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<512x512xbf16>>
          %2 = hal.interface.binding.subspan layout(#pipeline_layout) binding(2) alignment(64) offset(%c0) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<512x512xbf16>>
          ...
          %4 = iree_tensor_ext.dispatch.tensor.load %1, offsets = [0, 0], sizes = [512, 512], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<512x512xbf16>> -> tensor<512x512xbf16>
          %5 = tensor.empty() : tensor<512x512xbf16>
          %6 = tensor.empty() : tensor<512x512xf32>
          %7 = linalg.fill ins(%cst : f32) outs(%6 : tensor<512x512xf32>) -> tensor<512x512xf32>
          %8 = linalg.matmul indexing_maps = [#map, #map1, #map2] ins(%3, %4 : tensor<512x512xbf16>, tensor<512x512xbf16>) outs(%7 : tensor<512x512xf32>) -> tensor<512x512xf32>
          %9 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel"]} ins(%8 : tensor<512x512xf32>) outs(%5 : tensor<512x512xbf16>) {
          ^bb0(%in: f32, %out: bf16):
            %10 = arith.truncf %in : f32 to bf16
            linalg.yield %10 : bf16
          } -> tensor<512x512xbf16>
          iree_tensor_ext.dispatch.tensor.store %9, %2, offsets = [0, 0], sizes = [512, 512], strides = [1, 1] : tensor<512x512xbf16> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<512x512xbf16>>
          return
        }
      }
    }
  }
  util.func public @mm_512$async(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view, %arg2: !hal.fence, %arg3: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    %c0 = arith.constant 0 : index
    %c524288 = arith.constant 524288 : index
    %c512 = arith.constant 512 : index
    %element_type_bf16 = hal.element_type<bf16> : i32
    %dense_row_major = hal.encoding_type<dense_row_major> : i32
    hal.buffer_view.assert<%arg0 : !hal.buffer_view> message("tensor") shape([%c512, %c512]) type(%element_type_bf16) encoding(%dense_row_major)
    %0 = stream.tensor.import on(#hal.device.affinity<@__device_0>) %arg0 : !hal.buffer_view -> tensor<512x512xbf16> in !stream.resource<external>{%c524288}
    %1 = stream.timepoint.import on(#hal.device.affinity<@__device_0>) %arg2 : (!hal.fence) => !stream.timepoint
    hal.buffer_view.assert<%arg1 : !hal.buffer_view> message("tensor") shape([%c512, %c512]) type(%element_type_bf16) encoding(%dense_row_major)
    %2 = stream.tensor.import on(#hal.device.affinity<@__device_0>) %arg1 : !hal.buffer_view -> tensor<512x512xbf16> in !stream.resource<external>{%c524288}
    %result, %result_timepoint = stream.resource.alloca uninitialized on(#hal.device.affinity<@__device_0>) await(%1) => !stream.resource<external>{%c524288} => !stream.timepoint
    %3 = stream.cmd.execute on(#hal.device.affinity<@__device_0>) await(%result_timepoint) => with(%0 as %arg4: !stream.resource<external>{%c524288}, %2 as %arg5: !stream.resource<external>{%c524288}, %result as %arg6: !stream.resource<external>{%c524288}) {
      stream.cmd.dispatch @mm_512$async_dispatch_0::@rocm_hsaco_fb::@mm_512$async_dispatch_0_matmul_512x512x512_bf16xbf16xf32 {
    ...
    %4 = stream.tensor.export on(#hal.device.affinity<@__device_0>) %result : tensor<512x512xbf16> in !stream.resource<external>{%c524288} -> !hal.buffer_view
    util.return %4 : !hal.buffer_view
  }
  util.func public @mm_512(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub} {
    %0 = util.null : !hal.fence
    %c-1_i32 = arith.constant -1 : i32
    %c0 = arith.constant 0 : index
    %device_0 = hal.devices.get %c0 : !hal.device
    %fence = hal.fence.create device(%device_0 : !hal.device) flags("None") : !hal.fence
    %1 = util.call @mm_512$async(%arg0, %arg1, %0, %fence) : (!hal.buffer_view, !hal.buffer_view, !hal.fence, !hal.fence) -> !hal.buffer_view
